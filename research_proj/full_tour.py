...
def main():
    ...
    start = timer()
    tournament_default = axl.Tournament(players, turns=200, repetitions=1000)
    tournament_mc = axl.Tournament(players, turns=200, deviation=40, repetitions=1000)
    end = timer()
    print("time spent for init: {}".format(end - start))
    ### PLAY ###
    start = timer()
    result_set_default = tournament_default.play(filename="tournament_default.csv",processes=4) #
    ...
    ### EXPORT ###
    start = timer()
    matrix_default = axl.ResultMatrix(filename="tournament_default.csv", players=players, repetitions=1000, tour_type="default_devNone")
    matrix_mc = axl.ResultMatrix(filename="tournament_mc.csv", players=players, repetitions=1000, tour_type="montecarlo_dev40")
